...
read -p "(برای مثال root) نام کاربری سرور خود را به انگلیسی وارد کنید: " ssh_user

echo
read -s -p "(مثال swordfish) رمز عبور سرور خود را وارد کنید: " ssh_pass
echo

echo
read -p "(مثال omid@akherati.ir) آدرس ایمیل خود را برای دریافت گواهی SSL وارد کنید: " email
